...
def load_data(prefix):
    ...
    """
    train_path = f"{prefix}_train.txt"
    test_path = f"{prefix}_test.txt"
...
class LanguageModel(object):
    ...
    def perplexity(self, test_data):
        ...
        N = len([tok for tok in test_tokens if tok is not None])

        known_ngrams = (self._convert_oov(ngram) for ngram in test_ngrams if ngram is not None)
...
if __name__ == '__main__':
    ...
    for pre in ['BPE', 'UNI', 'WPC']:
        train, test = load_data(pre)
